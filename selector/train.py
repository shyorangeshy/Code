    ...
    for epoch in range(checkpoint, args.epoch_nums):
        ...
        for step, batch in tqdm(enumerate(train_dataloader),desc = "Training", total = batch_nums, ncols=50):
            ...
            if global_step % args.save_step == 0:
                new_eva,_= dev(model, dev_dataloader, 'dev',
                ...
                if new_eva[-1] > best_f1:
                    ...
                    new_eva,acc_score = dev(model, test_dataloader, 'test',
                                           str(epoch) + '-' + str(global_step))
                    fitlog.add_metric(new_eva[-1], name="test_new F1", step=global_step)
                    best_res = (str(epoch) + '-' + str(global_step),acc_score, [new_eva])
                else:
                    new_eva,_ = dev(model, test_dataloader, 'test',
                                           str(epoch) + '-' + str(global_step))
                    fitlog.add_metric(new_eva[-1], name="test_new F1", step=global_step)
                model.train()
        ...
        new_eva,_ = dev(model, dev_dataloader, 'dev',str(epoch) + '-' + str(global_step))
        if new_eva[-1] > best_f1:
...
def dev(model, dataloader, prefix='', suffix=''):
    ...
    acc_score=accuracy_score(total_gt_labels, total_pt_labels)
    ...
    return [new_p, new_r, new_f1],acc_score
